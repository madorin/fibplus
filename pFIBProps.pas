...
type
  ...
  TpPrepareOption = (pfSetRequiredFields, pfSetReadOnlyFields, pfImportDefaultValues, psUseBooleanField, psUseGuidField,
    psSQLINT64ToBCD, psApplyRepositary, psGetOrderInfo, psAskRecordCount, psCanEditComputedFields, psSetEmptyStrToNull,
    psSupportUnicodeBlobs, psUseLargeIntField);
  TpPrepareOptions = set of TpPrepareOption;

  TpFIBDsOption = (poTrimCharFields, poRefreshAfterPost, poRefreshDeletedRecord, poStartTransaction, poAutoFormatFields,
    poProtectedEdit, poUseSelectForLock, poKeepSorting,
{$IFDEF OBSOLETE_PROPS}
    poAllowChangeSqls,
{$ENDIF}
    poPersistentSorting, poVisibleRecno, poNoForceIsNull, poFetchAll, poFreeHandlesAfterClose, poCacheCalcFields,
    poRefreshAfterDelete, poDontCloseAfterEndTransaction);
  TpFIBDsOptions = set of TpFIBDsOption;

  TpFIBQueryOption  = (qoStartTransaction, qoAutoCommit, qoTrimCharFields, qoNoForceIsNull, qoFreeHandleAfterExecute);
  ...
  published
    property DateTimeDisplayFormat: string read FDisplayFormatDateTime write FDisplayFormatDateTime stored StoreDfDt;
    property NumericDisplayFormat: string read FDisplayFormatNumeric write FDisplayFormatNumeric stored StoreDfN;
    property NumericEditFormat: string read FEditFormatNumeric write FEditFormatNumeric stored StoreEfN;
    property DisplayFormatDate: string read FDisplayFormatDate write FDisplayFormatDate stored StoreDfD;
    property DisplayFormatTime: string read FDisplayFormatTime write FDisplayFormatTime stored StoreDfT;
  end;
...
{$ENDIF}
  ...
  published
    ...
    property UseGeneratorCache: TUseGeneratorCache read FUseGeneratorCache write FUseGeneratorCache default No;
...
{$IFDEF CSMonitor}
  ...
  published
    ...
    property IncludeDatasetDescription: boolean read FIncludeDatasetDescription write SetIncludeDatasetDescription
      default True;
  end;
{$ENDIF}
...
begin
  if Dest is TAutoUpdateOptions
  then
    with TAutoUpdateOptions(Dest) do begin
...
begin
  ...
  if FSelectGenID
  then FWhenGetGenID := wgBeforePost
  else FWhenGetGenID := wgNever;
end;
...
begin
  ...
  if FGenBeforePost
  then FWhenGetGenID := wgBeforePost
  else FWhenGetGenID := wgOnNewRecord
end;
...
begin
  ...
  if (FGeneratorName = '') and (WhenGetGenID <> wgNever)
  then begin
    MT := GetModifiedTable;
    if (Length(MT) > 0) and (MT[1] = '"')
    then begin
      MT := FastCopy(MT, 2, Length(MT) - 2);
      FGeneratorName := '"' + DefPrefixGenName + MT + DefSufixGenName + '"'
    end
    else FGeneratorName := DefPrefixGenName + MT + DefSufixGenName;
  end;
  FModified := True;
end;
...
begin
  if not Value
  then FWhenGetGenID := wgNever
  else if FWhenGetGenID = wgNever
  then FWhenGetGenID := wgBeforePost;
end;
...
procedure TAutoUpdateOptions.SetReturningFields(const Value: TSetReturningFields);
begin
...
begin
  ...
  else if Assigned(FOwner) and (csDesigning in FOwner.ComponentState) and not CmpInLoadedState(FOwner)
  then begin
...
begin
  if FF_UseRuntimeDefaults
  then result := FDisplayFormatTime <> RDefTimeFormat
  else result := FDisplayFormatTime <> DefTimeFormat
end;

function TFormatFields.StoreDfD: boolean;
begin
  if FF_UseRuntimeDefaults
  then result := FDisplayFormatDate <> RDefDateFormat
  else result := FDisplayFormatDate <> DefDateFormat;
end;

function TFormatFields.StoreDfDt: boolean;
begin
  if FF_UseRuntimeDefaults
  then result := FDisplayFormatDateTime <> RDefDateFormat + ' ' + RDefTimeFormat
  else result := FDisplayFormatDateTime <> DefDateFormat + ' ' + DefTimeFormat;
end;

function TFormatFields.StoreDfN: boolean;
begin
  if FF_UseRuntimeDefaults
  then result := FDisplayFormatNumeric <> RDefDisplayFormatNum
  else result := FDisplayFormatNumeric <> '#,##0.';
end;

function TFormatFields.StoreEfN: boolean;
begin
  if FF_UseRuntimeDefaults
  then result := FEditFormatNumeric <> RDefEditFormatNum
  else result := FEditFormatNumeric <> '0.';
end;

procedure TFormatFields.AssignTo(Dest: TPersistent);
begin
  if Dest is TFormatFields
  then
    with TFormatFields(Dest) do begin
...
begin
  result := '';
  if Assigned(FOwner) and (FOwner is TFIBDataBase)
  then
...
begin
  if Assigned(FOwner) and (FOwner is TFIBDataBase)
  then
    with TFIBDataBase(FOwner) do DBParamByDPB[isc_dpb_user_name] := Value
end;
...
begin
  if Assigned(FOwner) and (FOwner is TFIBDataBase)
  then
    with TFIBDataBase(FOwner) do DBParamByDPB[isc_dpb_sql_role_name] := Value
end;
...
begin
  if Assigned(FOwner) and (FOwner is TFIBDataBase)
  then
    with TFIBDataBase(FOwner) do DBParamByDPB[isc_dpb_password] := Value
end;
...
begin
  if Assigned(FOwner) and (FOwner is TFIBDataBase)
  then
    with TFIBDataBase(FOwner) do DBParamByDPB[isc_dpb_lc_ctype] := FastUpperCase(Value)
end;
...
begin
  if Assigned(FOwner) and (FOwner is TFIBDataBase)
  then
    with TFIBDataBase(FOwner) do DBParamByDPB[isc_dpb_instance_name] := Value
end;
{$ENDIF}
{ TSQLs }
...
procedure TDBParams.DefineProperties(Filer: TFiler);
  ...
  begin
    ...
    then begin
      result := True;
      if Filer.Ancestor is TStrings
      then result := not Equals(TStrings(Filer.Ancestor))
    end
    else result := Count > 0;
  end;
...
var
  ...
  OldPassword: string;
begin
  if (FOwner is TFIBDataBase) and (ddoNotSavePassword in TFIBDataBase(FOwner).DesignDBOptions)
  then begin
...
begin
  if Index = Count
  then AddObject(S, TCondition.Create(Self))
  else inherited Put(Index, S)
end;
...
function TConditions.AddCondition(const Name, Condition: string; Enabled: boolean): TCondition;
var
  I: integer;
begin
  I := IndexOfName(Name);
...
begin
  ...
  if I < 0
  then result := nil
  else result := TCondition(Objects[I]);
end;
...
begin
  ...
  if result = nil
  then raise Exception.Create('Can''t find Condition ' + Name);
end;
...
begin
  ...
  then exit;
  for I := 0 to c do
    if TCondition(Objects[I]).Enabled and not IsBlank(Strings[I])
    then begin
      result := result + '(' + ValueFromStr(Strings[I]) + ')' + CLRF;
      result := result + CLRF + ' and ';
    end;
  if not IsBlank(result)
  then SetLength(result, Length(result) - 7);
end;
...
var
  Wh, Wh1: string;
begin
  with TFIBQuery(FFIBQuery) do begin
    BeginModifySQLText;
    Include(FState, csInApply);
    try
      Wh1 := Conditions.EnabledText;
      if IsBlank(FPrimarySQL)
      then FPrimarySQL := SQL.Text
      else RestorePrimarySQL;
      if not IsBlank(Wh1)
      then begin
        Wh := GetMainWhereClause;
        if Wh <> ''
        then Wh := Wh + CLRF + 'and ' + Wh1
        else Wh := Wh1;
...
begin
  if (FApplied) and (FState = [])
  then
    try
      Include(FState, csInCancel);
      RestorePrimarySQL
    finally Exclude(FState, csInCancel);
    end;
end;
...
begin
  ...
  try
    FApplied := False;
    if not IsBlank(FPrimarySQL)
    then
      if TFIBQuery(FFIBQuery).SQL.Text <> FPrimarySQL
      then TFIBQuery(FFIBQuery).SQL.Text := FPrimarySQL;
  finally Exclude(FState, csInRestorePrimarySQL);
  end
end;
...
begin
  ...
  then result := nil
  else result := TCondition(Objects[Index]);
end;

function TConditions.AddObject(const S: string; AObject: TObject): integer;
begin
  if (AObject is TCondition) or (AObject = nil)
  then result := inherited AddObject(S, AObject)
  else result := -1
end;
...
begin
  if not Condition[Index].FInDestroy
  then Condition[Index].Free
  else inherited;
end;
...
procedure TConditions.DefineProperties(Filer: TFiler);
  ...
  begin
    ...
    if (Filer.Ancestor <> nil)
    then begin
      if not(Filer.Ancestor is TConditions)
      then result := False
      else if not Equals(TStrings(Filer.Ancestor))
      then result := True
      else begin
        for I := 0 to Pred(Count) do
          if (TConditions(Filer.Ancestor).Condition[I].Enabled <> Condition[I].Enabled) or
            (TConditions(Filer.Ancestor).Condition[I].Name <> Condition[I].Name)
          then begin
            result := True;
            exit;
          end;
...
{$ELSE}
  ...
  if Assigned(FFIBQuery) and (csAncestor in FFIBQuery.ComponentState) and not((Filer is TReader) or (Filer is TWriter))
  then HasData := False
  else HasData := DoWriteConditions;
...
begin
  S := '[';
  for I := 0 to Pred(Count) do S := S + IntToStr(integer(Condition[I].Enabled)) + ',';
  S[Length(S)] := ']';
      ...
      then begin
        ...
        then begin
          ...
          then begin
            ...
            if Index >= 0
            then begin
              if Condition[Index].FEnabledFromStream
              then Condition[Index].SetEnabledFromStream(En);
              // Condition[Index].Enabled:=En;
              if (V <> '0') and Condition[Index].FValueFromStream
              then Condition[Index].SetValueFromStream(V);
...
begin
  ...
  for I := 0 to Count - 1 do begin
    ...
    then begin
      ...
      then begin
        ...
        V := ValueFromStr(Get(I));
        if V = ValueFromStr(TConditions(Writer.Ancestor).Strings[Index])
        then V := '0';
        if (V <> '0') or (Condition[I].Enabled <> TConditions(Writer.Ancestor).Condition[Index].Enabled)
        then begin
          Writer.WriteString(N);
          Writer.WriteString(V);
          Writer.WriteBoolean(Condition[I].Enabled);
  ...
  then begin
    ...
    for I := 0 to TConditions(Writer.Ancestor).Count - 1 do
      if FindCondition(TConditions(Writer.Ancestor).Condition[I].Name) = nil
      then begin
        Inc(j);
        if j = 1
        then Writer.WriteString('#@DELETED');
...
begin
  I := FOwner.IndexOfObject(Self);
  if FOwner.Strings[I] <> FOwner.Names[I] + '=' + Value
  then begin
...
procedure TAutoUpdateOptions.SetUpdateOnlyModifiedFields(const Value: boolean);
begin
  FUpdateOnlyModifiedFields := Value and FAutoReWriteSqls and FCanChangeSQLs;
end;
...
begin
  ...
  if FastUpperCase(Copy(FSwapDirectory, 1, Length(AppPathTemplate))) = AppPathTemplate
  then result := ApplicationPath + Copy(FSwapDirectory, Length(AppPathTemplate) + 1, MaxInt)
  else result := FSwapDirectory;
...
begin
  if Length(FModifiedTable) = 0
  then begin
    if FModifiedTableHaveAlias
    then begin
      ...
      if FOwner <> nil
      then FAliasModifiedTable := AliasForTable(TFIBDataSet(FOwner).QSelect.ReadySQLText(False), UpdateTableName)
      else FAliasModifiedTable := UpdateTableName;
    end;
    if Assigned(FOwner) and Assigned(TFIBDataSet(FOwner).Database)
    then
      with TFIBDataSet(FOwner).Database do begin
        FModifiedTable := EasyFormatIdentifier(SQLDialect, FModifiedTable, EasyFormatsStr);
        FAliasModifiedTable := EasyFormatIdentifier(SQLDialect, FAliasModifiedTable, EasyFormatsStr);
      end;
  end;
end;
...
begin
  ...
  if (FOwner is TFIBQuery) or (FOwner is TFIBDataSet)
  then FEnabled := csmeTransactionDriven
  else if FOwner is TFIBTransaction
  then FEnabled := csmeDatabaseDriven
  else if FOwner is TFIBDataBase
  then FEnabled := csmeDisabled;
...
begin
  if (FOwner is TFIBQuery) or (FOwner is TFIBDataSet)
  then result := FEnabled <> csmeTransactionDriven
  else if FOwner is TFIBTransaction
  then result := FEnabled <> csmeDatabaseDriven
  else if FOwner is TFIBDataBase
  then result := FEnabled <> csmeDisabled
  else result := False
end;

procedure TCSMonitorSupport.SetEnabled(const Value: TCSMonitorEnabled);
begin
  if (FOwner is TFIBTransaction) and not(Value in [csmeDisabled, csmeEnabled, csmeDatabaseDriven])
  then raise EFIBCSMonitorError.Create('This object cannot use this value');
  if (FOwner is TFIBDataBase) and not(Value in [csmeDisabled, csmeEnabled])
  then raise EFIBCSMonitorError.Create('This object cannot use this value');
  FEnabled := Value;
  if (FOwner is TFIBDataSet)
  then
    (FOwner as TFIBDataSet).SetCSMonitorSupportToQ;
end;
...
begin
  CacheFileName := TGeneratorsCache(THackOwnedCollection(Collection).GetOwner).FCacheFileName;
  if Length(CacheFileName) > 0
  then begin
...
begin
  if Length(FCacheFileName) = 0
  then result := False
  else
    case FUseGeneratorCache of
      No: result := False;
      forAll: result := True;
    else result := Find(GenName) <> nil
    end
end;
...
begin
  ...
  if I < 0
  then result := nil
  else result := TGeneratorParams(FGenerators.Items[I]);
end;
...
begin
  ...
  if GenParams <> nil
  then result := GenParams.NextValue
  else result := -1;
end;
...
begin
  for result := 0 to FGenerators.Count - 1 do
    if AnsiCompareText(TNamedItem(FGenerators.Items[result]).Name, AName) = 0
    then exit;
  result := -1;
end;
  ...
  then begin
    ...
    try
      ...
      for I := 0 to ts.Count - 1 do begin
        curGen := Find(ts[I]);
        if curGen = nil
        then curGen := TGeneratorParams(FGenerators.Add);
        curGen.Name := ts[I];
  ...
  then begin
    ...
    try
      for I := 0 to FGenerators.Count - 1 do
        with TGeneratorParams(FGenerators.Items[I]) do begin
          Ini.WriteInteger(Name, 'StepForCache', FStepForCache);
          Ini.WriteString(Name, 'NextValue', IntToStr(FNextValue));
          Ini.WriteString(Name, 'Limit', IntToStr(FLimit));
        end;
...
procedure TGeneratorsCache.InternalSetGenParams(const GenName: string; Limit: Int64);
var
  GenParams: TGeneratorParams;
begin
  GenParams := Find(GenName);
...
begin
  GenParams := Find(GenName);
  if GenParams <> nil
  then result := GenParams.FStepForCache
  else result := DefaultStep
end;

procedure TGeneratorsCache.AddGenerator(const GenName: string; Limit: Int64);
var
  GenParams: TGeneratorParams;
begin
  GenParams := Find(GenName);
